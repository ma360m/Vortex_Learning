...
export function SearchConsole({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="rounded-3xl border border-white/60 bg-white/88 p-3 shadow-[0_26px_90px_rgba(9,29,83,0.16)] backdrop-blur-xl">
      <label className="relative block">
        <Search className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-vortex-blue" />
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search courses, subjects, tutors, articles, past papers, notes, FAQs"
          className="h-14 w-full rounded-2xl border border-vortex-border bg-white pl-12 pr-4 text-base font-medium text-vortex-navy outline-none transition placeholder:text-vortex-muted focus:border-vortex-cyan focus:ring-4 focus:ring-vortex-cyan/15"
        />
      </label>
          ...
          results.map((result) => {
            ...
            return (
              <Link
                ...
                href={result.href}
                className="group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border border-transparent px-3 py-3 transition hover:border-vortex-border hover:bg-vortex-soft"
              >
